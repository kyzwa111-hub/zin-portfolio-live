...
function RequestHistory() {
  ...
  return <section className="admin-card admin-wide-card"><div className="admin-card-heading"><div><span className="admin-eyebrow">Calculator security</span><h2>Telegram access history</h2></div><span className="admin-count">{requests.data?.length ?? 0} requests</span></div><p className="admin-section-note">Review who asked to use the calculator. Revoking an approved request immediately makes its access token invalid on the next status check.</p>{requests.isLoading ? <div className="admin-empty"><Loader2 className="spin" size={18} /> Loading request history…</div> : requests.data?.length ? <div className="request-list">{requests.data.map((request) => <div className="request-row" key={request.requestId}><div><div className="request-meta"><span>{request.telegramUsername ? `@${request.telegramUsername}` : "Telegram user not recorded"}</span><time>{new Date(request.createdAt).toLocaleString()}</time></div><code>{request.requestId}</code></div><div className="request-actions"><strong className={`request-status ${request.status}`}>{request.status}</strong>{(request.status === "approved" || request.status === "pending") && <button className="admin-revoke" onClick={() => { if (window.confirm("Revoke this calculator access request?")) revoke.mutate({ requestId: request.requestId }); }} disabled={revoke.isPending}><UserX size={13} /> Revoke</button>}</div></div>)}</div> : <div className="admin-empty">No Telegram access requests yet.</div>}</section>;
}
