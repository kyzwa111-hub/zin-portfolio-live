...
export function calculateFyAnnualGross(monthlyIncomes: number[], fallbackMonthlyGross: number) {
  ...
  return enteredTotal > 0 ? enteredTotal : Math.max(0, fallbackMonthlyGross) * 12;
}
export const formatFinancialYear = (year: string) => year.startsWith("FY ") ? year : `FY ${year}`;
...
function downloadPayslipPdf(result: PayslipResult | null, rules: TaxRule, taxMode: TaxMode, financialYear: string) {
  if (!result) return;
  ...
  pdf.text(financialYear, 188, 28, { align: "right" });
  pdf.text(`Financial year: ${financialYear}`, left, 52);
  pdf.text(`Basis: ${rules.effective.replaceAll("–", "-")}`, left, 59);
  pdf.text(`Tax mode: ${taxMode === "employee" ? "Employee-borne PIT" : "Employer-borne PIT"}`, left, 66);
  ...
  pdf.save(`zin-min-htet-payslip-${financialYear}.pdf`);
}
...
export default function PayrollCalculator() {
  ...
  const [fyIncome, setFyIncome] = useState<Record<string, string>>(() => Object.fromEntries(FY_MONTHS.map((month) => [month, ""])));

  const result = useMemo(() => {
    if (!accessGranted) return null;
    const monthlyGross = numberValue(basicSalary) + numberValue(allowance) + numberValue(overtime);
    const fyIncomeValues = FY_MONTHS.map((month) => numberValue(fyIncome[month] ?? ""));
    const annualGross = calculateFyAnnualGross(fyIncomeValues, monthlyGross) + numberValue(annualBonus) + numberValue(otherEarnings);
    const { employeeSSB, employerSSB } = calculateSSB(monthlyGross);
    const annualEmployeeSSB = fyIncomeValues.some((income) => income > 0) ? fyIncomeValues.reduce((sum, income) => sum + calculateSSB(income).employeeSSB, 0) : employeeSSB * 12;
    const annualEmployerSSB = fyIncomeValues.some((income) => income > 0) ? fyIncomeValues.reduce((sum, income) => sum + calculateSSB(income).employerSSB, 0) : employerSSB * 12;
    ...
    const reliefArgs = [parentCount, spouseCount, childCount, numberValue(lifeInsurance), annualEmployeeSSB, numberValue(otherDeductions)] as const;
    ...
    const taxableIncome = Math.max(0, taxableGross - personalRelief - parentCount * rules.parentRelief - spouseCount * rules.spouseRelief - childCount * rules.childRelief - numberValue(lifeInsurance) - Math.min(annualEmployeeSSB, 72_000) - numberValue(otherDeductions));
  ...
  const requestAccess = () => {
    ...
    accessRequest.mutate({ requesterName: requesterName.trim() });
  };
  ...
  return (
    <section className="calculator-section section-pad" id="calculator">
      ...
      {accessGranted && <PayrollFormDownloads data={result ? { monthlyGross: result.monthlyGross, annualGross: result.annualGross, employeeSSB: result.employeeSSB, employerSSB: result.employerSSB, employeeSSBAnnual: result.annualEmployeeSSB, employerSSBAnnual: result.annualEmployerSSB, monthlyPIT: result.monthlyPIT, annualPIT: result.annualPIT, monthlyNet: result.monthlyNet, employerCost: result.employerCost, taxableIncome: result.taxableIncome, lifeInsurance: numberValue(lifeInsurance), otherDeductions: numberValue(otherDeductions), parents: numberValue(parents), spouse: numberValue(spouse), children: numberValue(children), taxLabel: rules.label, taxEffective: rules.effective, taxMode, financialYear: formatFinancialYear(taxVersion) } : null} />}
