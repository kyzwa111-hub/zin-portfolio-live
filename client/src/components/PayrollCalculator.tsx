...
export default function PayrollCalculator() {
  ...
  const accessRequest = trpc.calculatorAccess.request.useMutation({ onSuccess: (data) => setAccess({ requestId: data.requestId, token: data.token }) });
  ...
  const result = useMemo(() => {
    ...
    const ssbBase = Math.min(monthlyGross, 300_000);
    const employeeSSB = ssbBase * 0.02;
    const employerSSB = ssbBase * 0.02;
